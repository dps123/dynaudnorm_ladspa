...
namespace DYNAUDNORM_NS
{
	...
	public:
		...
		inline DAN_Data *data(const uint32_t &channel)
		{
			assert(channel < m_channels);
			return m_data[channel];
		}

		inline const DAN_Data *data(const uint32_t &channel) const
		{
			assert(channel < m_channels);
			return m_data[channel];
		}
		...
		inline void write(const DAN_Data *const *const src, const uint32_t &srcOffset, const uint32_t &destOffset, const uint32_t &length)
		{
			assert(length + destOffset <= m_frameLength);
			for (uint32_t c = 0; c < m_channels; c++)
			{
				memcpy(&m_data[c][destOffset], &src[c][srcOffset], length * sizeof(DAN_Data));
			}
		}
		...
		inline void write(const FrameData *const src, const uint32_t &srcOffset, const uint32_t &destOffset, const uint32_t &length)
		{
			...
			for (uint32_t c = 0; c < m_channels; c++)
			{
				memcpy(&m_data[c][destOffset], &src->data(c)[srcOffset], length * sizeof(DAN_Data));
			}
		}

		inline void read(DAN_Data *const *const dest, const uint32_t &destOffset, const uint32_t &srcOffset, const uint32_t &length)
		{
			assert(length + srcOffset <= m_frameLength);
			for (uint32_t c = 0; c < m_channels; c++)
			{
				memcpy(&dest[c][destOffset], &m_data[c][srcOffset], length * sizeof(DAN_Data));
			}
		}
		...
		inline void read(FrameData *const dest, const uint32_t &destOffset, const uint32_t &srcOffset, const uint32_t &length)
		{
			...
			for (uint32_t c = 0; c < m_channels; c++)
			{
				memcpy(&dest->data(c)[destOffset], &m_data[c][srcOffset], length * sizeof(DAN_Data));
			}
		}
	...
	public:
		...
		inline void putSamples(const DAN_Data *const *const src, const uint32_t &srcOffset, const uint32_t &length)
		{
		...
		inline void getSamples(DAN_Data *const *const dest, const uint32_t &destOffset, const uint32_t &length)
		{
